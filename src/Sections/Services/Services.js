...
const Services = () => {
  ...
  return (
    ...
    return (
        ...
        viewport={{ once: true }}
        whileInView={{ opacity: 1, x: 0 }}></motion.img>
      </div>
          );
        })
      : 
      serviceData.map((service, index) => {
        ...
        whileInView="visible"
        viewport={{ once: true }}
        className="seperator 2xl:h-[500px] lg:h-[600px] medium:hidden"></motion.div>
        <motion.img 
        src={`${process.env.PUBLIC_URL}/${service.image}`} 
        ...
        className="w-[50%]"
        viewport={{ once: true }}>
        <img src={`${process.env.PUBLIC_URL}/${service.image}`} className="w-[500px] mr-[50px] medium:m-auto"></img>
        </motion.div>
        <motion.div
        variants={fadeBottom}
        viewport={{ once: true }}
        initial="hidden"
        whileInView="visible"
        className="seperator 2xl:h-[500px] lg:h-[600px] medium:hidden ml-[40px]"></motion.div>
        <motion.div
        variants={fadeRight}
        viewport={{ once: true }}
        initial="hidden"
